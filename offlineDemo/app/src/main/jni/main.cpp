...
JavaVM *global_JavaVM = NULL;

void dispatchAudioBuffer(char *buff, int buff_size)
{
    if(global_JavaVM != NULL)
    {
        JNIEnv *env = NULL;
        global_JavaVM->AttachCurrentThread(&env, NULL);
        if(env != NULL)
        {
            jclass javaClazz = env->FindClass("com/turing/tts/TuringTTS");
            if(javaClazz == NULL)
            {
                LOGE("find javaClazz failed in dispatchAudioBuffer.");
                return;
            }

            jmethodID methodID = env->GetStaticMethodID(javaClazz, "onAudioReadyCallback", "([BI)V");
            if(methodID == NULL)
            {
                LOGE("find onAudioReadyCallback failed in dispatchAudioBuffer.");
                env->DeleteLocalRef(javaClazz);
                return;
            }

            if(buff_size == -1)  // end of audio, "EOF"
            {
                jbyteArray jbuff = env->NewByteArray(3);
                if(jbuff == NULL)
                {
                    LOGE("create new byte array failed in dispatchAudioBuffer.");
                    env->DeleteLocalRef(javaClazz);
                    return;
                }

                env->SetByteArrayRegion(jbuff, 0, 3, (jbyte*)buff);
                env->CallStaticVoidMethod(javaClazz, methodID, jbuff, -1);
            ...
            {
                jbyteArray jbuff = env->NewByteArray(buff_size);
                if(jbuff == NULL)
                {
                    LOGE("create new byte array failed in dispatchAudioBuffer.");
                    env->DeleteLocalRef(javaClazz);
                ...
                env->SetByteArrayRegion(jbuff, 0, buff_size, (jbyte*)buff);
                env->CallStaticVoidMethod(javaClazz, methodID, jbuff, buff_size);
                env->DeleteLocalRef(jbuff);
        ...
        {
            LOGE("env is null in dispatchAudioBuffer.");
        }
    }
    else
    {
        LOGE("jvm is null in dispatchAudioBuffer.");
    }
}
...
jint JNIEXPORT JNICALL Java_com_turing_tts_TuringTTS_jniInit(
    JNIEnv* env,
    jobject thiz)
{
    if(env->GetJavaVM(&global_JavaVM) != 0)
    {
        LOGE("get JVM failed");
        return -5;
    }
...
-5:init tts failed
 ...
 1:succeed
*/
jint JNIEXPORT JNICALL Java_com_turing_tts_TuringTTS_jniInit2(
    JNIEnv* env,
    jclass,
    jobject context,
    jstring japikey,
...
jboolean JNIEXPORT JNICALL Java_com_turing_tts_TuringTTS_jniFree(
    JNIEnv* env,
    jobject thiz)
{
...
void JNIEXPORT JNICALL Java_com_turing_tts_TuringTTS_jniStop(
    JNIEnv* env,
    jobject thiz)
{
    //LOGI("jniStop called");
...
jboolean JNIEXPORT JNICALL Java_com_turing_tts_TuringTTS_jniSynthAudio(
    JNIEnv* env,
    jobject thiz,
    jstring jtext,
    jint syn_status)
...
/*
    if(get_status() != 1)
    {
    	LOGE("invalid initialization");
        return false;
    }
*/
    if(syn_status <= 0)
    {
        LOGI("synthesize skipped");
        return false;
    }
    ...
    synthAudio(pHeap, tResPackDesc, text);
    env->ReleaseStringUTFChars(jtext, text);
    // end of audio
    if(global_bStopTTS == emFalse)
    {
        dispatchAudioBuffer("EOF", -1);
    }
    return true;
}

void JNIEXPORT JNICALL Java_com_turing_tts_TuringTTS_jniSynthFromParams(
    JNIEnv* env,
    jobject thiz,
    jbyteArray params,
    jint length)
{
    jbyte *pBytes = env->GetByteArrayElements(params, NULL);
    const char *pp = (char*)pBytes;
    tts_vocoder(pp, length);
    env->ReleaseByteArrayElements(params, pBytes, JNI_ABORT);
}
